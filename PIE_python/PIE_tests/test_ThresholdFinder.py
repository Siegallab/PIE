...
class TestSetThresholdWindowSize(unittest.TestCase):
	...
	def test_default_window_size(self):
		...
		long enough
		'''
		self.threshold_finder_standin.ln_tophat_hist = range(0, 64)
		expected_window_size = 21
		test_smooth_window_size = \
			self.threshold_finder_standin._set_smoothing_window_size()
	...
	def test_window_size_reductions(self):
		...
		for hist_length, expected_window_size in hist_lengths_window_sizes:
			self.threshold_finder_standin.ln_tophat_hist = range(0, hist_length)
			test_smooth_window_size = \
				self.threshold_finder_standin._set_smoothing_window_size()
			self.assertEqual(expected_window_size, test_smooth_window_size)
...
class TestSmoothLogHistogram(unittest.TestCase):
	...
	def setUp(self):
		...
		hist_array_data = \
			np.loadtxt('PIE_tests/test_ims/tophat_im_small_best_hist.csv',
				delimiter=',')
		self.threshold_finder_standin.ln_tophat_hist = hist_array_data[1]
	...
	def test_histogram_smoothing(self):
		...
		'''
		self.threshold_finder_standin._smooth_log_histogram()
		assert_allclose(self.expected_ln_tophat_smooth,
			self.threshold_finder_standin.ln_tophat_smooth,
			atol = 10**-10)
